...
public:
  ...
  DiLeptonBuilder(const edm::ParameterSet&);
  ~DiLeptonBuilder() {};

  std::unique_ptr<DiLeptonInfoCollection> build(
...
template<typename Lepton, typename Fitter>
DiLeptonBuilder::DiLeptonBuilder(const edm::ParameterSet& cfg):
...
template<typename Lepton, typename Fitter>
std::unique_ptr<DiLeptonInfoCollection> DiLeptonBuilder::build(
  const LeptonCollection& leptons,
  DiLeptonCache& cache) const {

  auto ret_value = std::make_unique<DiLeptonInfoCollection>();

  for(size_t il1 = 0; il2 < leptons.size(); ++il2) {
