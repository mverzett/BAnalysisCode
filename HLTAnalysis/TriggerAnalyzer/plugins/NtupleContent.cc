...
void NtupleContent::SetNtupleVariables(TString Vars){
  ...
  if (Lite || Flat){
   ...
   t1->Branch("muon_phi",&muon_phi);
   t1->Branch("muon_charge",&muon_charge); 
   t1->Branch("muon_dxy",&muon_dxy);
   t1->Branch("muon_edxy",&muon_edxy); 
   t1->Branch("muon_soft",&muon_soft);
   t1->Branch("muon_trgIndex",&muon_trgIndex); 
   t1->Branch("nelectron",&nel);
   t1->Branch("el_pt",&el_pt); 
   t1->Branch("el_eta",&el_eta);
   t1->Branch("el_phi",&el_phi); 
   t1->Branch("el_charge",&el_charge);
   t1->Branch("el_mva_unbiased",&el_mva_unbiased);
   t1->Branch("el_islowpt",&el_islowpt);
   if (writeKll){
     if (Flat){
       t1->Branch("NRb_pt",&NRb_pt); 
       t1->Branch("NRb_eta",&NRb_eta);
       t1->Branch("NRb_phi",&NRb_phi); 
       t1->Branch("NRb_Kpt",&NRb_Kpt);
       t1->Branch("NRb_Keta",&NRb_Keta); 
       t1->Branch("NRb_Kphi",&NRb_Kphi);
       t1->Branch("NRb_l1pt",&NRb_l1pt); 
       t1->Branch("NRb_l1eta",&NRb_l1eta);
       t1->Branch("NRb_l1phi",&NRb_l1phi); 
       t1->Branch("NRb_l2pt",&NRb_l2pt); 
       t1->Branch("NRb_l2eta",&NRb_l2eta);  
       t1->Branch("NRb_l2phi",&NRb_l2phi);
     } else {
  ...
  t1->Branch("beam_ey",&beam_ey); 
  t1->Branch("beam_ez",&beam_ez);
  ...
  t1->Branch("vertex_y",&vertex_y);
  t1->Branch("vertex_z",&vertex_z); 
  t1->Branch("vertex_ex",&vertex_ex);
  t1->Branch("vertex_ey",&vertex_ey); 
  t1->Branch("vertex_ez",&vertex_ez);
  ...
  //gen
  if (writeGen){
   t1->Branch("ngenB",&ngenB); 
   t1->Branch("genB_pt",&genB_pt);
   t1->Branch("genB_phi",&genB_phi); 
   t1->Branch("genB_eta",&genB_eta);
   t1->Branch("genB_pdgId",&genB_pdgId); 
   t1->Branch("genB_Bindex",&genB_Bindex);
   ...
   t1->Branch("ngenLep",&ngenLep); 
   t1->Branch("genLep_pt",&genLep_pt);
   t1->Branch("genLep_phi",&genLep_phi); 
   t1->Branch("genLep_eta",&genLep_eta);
   t1->Branch("genLep_pdgId",&genLep_pdgId); 
   t1->Branch("genLep_mom",&genLep_mom);
   }
  //muons
  t1->Branch("nmuon",&nmuons); 
  t1->Branch("muon_pt",&muon_pt);
  ...
  t1->Branch("muon_phi",&muon_phi);
  t1->Branch("muon_charge",&muon_charge); 
  t1->Branch("muon_qual",&muon_qual);
  t1->Branch("muon_dxy",&muon_dxy); 
  t1->Branch("muon_dz",&muon_dz);
  t1->Branch("muon_edxy",&muon_edxy); 
  t1->Branch("muon_edz",&muon_edz);
  t1->Branch("muon_d0",&muon_d0); 
  t1->Branch("muon_ed0",&muon_ed0);
  t1->Branch("muon_vx",&muon_vx); 
  t1->Branch("muon_vy",&muon_vy);
  t1->Branch("muon_vz",&muon_vz); 
  t1->Branch("muon_iso",&muon_iso);
  ...
  t1->Branch("muon_soft",&muon_soft); 
  t1->Branch("muon_loose",&muon_loose);
  ...
  t1->Branch("el_phi",&el_phi);
  t1->Branch("el_charge",&el_charge); 
  t1->Branch("el_dxy",&el_dxy);
  t1->Branch("el_dz",&el_dz); 
  t1->Branch("el_edxy",&el_edxy);
  t1->Branch("el_edz",&el_edz); 
  t1->Branch("el_vx",&el_vx);
  t1->Branch("el_vy",&el_vy); 
  t1->Branch("el_vz",&el_vz);
  t1->Branch("el_mva_out",&el_mva_out); 
  t1->Branch("el_mva_iso",&el_mva_iso);
  t1->Branch("el_iso",&el_iso); 
  t1->Branch("el_veto",&el_veto);
  t1->Branch("el_soft",&el_soft); 
  t1->Branch("el_medium",&el_medium);
  t1->Branch("el_tight",&el_tight); 
  ...
  t1->Branch("el_islowpt",&el_islowpt);
  if (writeLowPtEl){
   t1->Branch("lowel_pt",&lowel_pt); 
   t1->Branch("lowel_eta",&lowel_eta);
   t1->Branch("lowel_phi",&lowel_phi); 
   t1->Branch("lowel_charge",&lowel_charge);
   t1->Branch("lowel_vx",&lowel_vx); 
   t1->Branch("lowel_vy",&lowel_vy);
   t1->Branch("lowel_vz",&lowel_vz); 
   t1->Branch("lowel_mva_unbiased",&lowel_mva_unbiased);
   t1->Branch("lowel_trkpt",&lowel_trkpt); 
   t1->Branch("lowel_trketa",&lowel_trketa);
   t1->Branch("lowel_trkphi",&lowel_trkphi);
  }
  if (writeLowPtGsf){
   t1->Branch("lowgsf_pt",&lowgsf_pt); 
   t1->Branch("lowgsf_eta",&lowgsf_eta);
   t1->Branch("lowgsf_phi",&lowgsf_phi); 
   t1->Branch("lowgsf_charge",&lowgsf_charge);
   t1->Branch("lowgsf_vx",&lowgsf_vx); 
   t1->Branch("lowgsf_vy",&lowgsf_vy);
   t1->Branch("lowgsf_vz",&lowgsf_vz); 
   t1->Branch("lowgsf_mva_unbiased",&lowgsf_mva_unbiased);
   t1->Branch("lowgsf_modept",&lowgsf_modept); 
   t1->Branch("lowgsf_modeeta",&lowgsf_modeeta);
   t1->Branch("lowgsf_modephi",&lowgsf_modephi);
  }
 if (writePFel){
    t1->Branch("pfel_pt",&pfel_pt); 
    t1->Branch("pfel_eta",&pfel_eta);
    t1->Branch("pfel_phi",&pfel_phi);
    t1->Branch("pfel_pfcharge",&pfel_charge);
    t1->Branch("pfel_vx",&pfel_vx); 
    t1->Branch("pfel_vy",&pfel_vy);
    t1->Branch("pfel_vz",&pfel_vz); 
    t1->Branch("pfel_soft",&pfel_soft);
    t1->Branch("pfel_medium",&el_medium); 
  ...
  //jets
  t1->Branch("jet_pt",&jet_pt); 
  t1->Branch("jet_eta",&jet_eta);
  t1->Branch("jet_phi",&jet_phi); 
  t1->Branch("jet_cEmEF",&jet_cEmEF);
  t1->Branch("jet_cHEF",&jet_cHEF); 
  t1->Branch("jet_cHMult",&jet_cHMult);
  t1->Branch("jet_cMuEF",&jet_cMuEF); 
  t1->Branch("jet_cMult",&jet_cMult);
  t1->Branch("jet_MuEF",&jet_MuEF); 
  t1->Branch("jet_nEmEF",&jet_nEmEF);
  t1->Branch("jet_nHEF",&jet_nHEF); 
  t1->Branch("jet_nMult",&jet_nMult);
  t1->Branch("jet_eEF",&jet_eEF); 
  t1->Branch("jet_pEF",&jet_pEF);
  //met
...
if (writePhill){
   ...
   t1->Branch("NRbphi_mass",&NRbphi_mass); 
   t1->Branch("NRbphi_charge",&NRbphi_charge);
   ...
   t1->Branch("NRbphi_lep1Id",&NRbphi_lep1Id); 
   t1->Branch("NRbphi_lep2Id",&NRbphi_lep2Id);
   ...
   t1->Branch("NRbphi_mll",&NRbphi_mll); 
   t1->Branch("NRbphi_phimass",&NRbphi_phimass);
   t1->Branch("NRbphi_K1UNFITpt_eta_phi",&NRbphi_K1UNFITpt_eta_phi);
   t1->Branch("NRbphi_K2UNFITpt_eta_phi",&NRbphi_K2UNFITpt_eta_phi);
   t1->Branch("NRbphi_k1_sdxy",&NRbphi_k1_sdxy); 
   t1->Branch("NRbphi_k2_sdxy",&NRbphi_k2_sdxy);
}
}
