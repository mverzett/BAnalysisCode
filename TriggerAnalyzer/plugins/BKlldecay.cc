...
void BKlldecay::Fill(NtupleContent &nt){
    ...
    for(const std::pair<unsigned int, unsigned int> &imu : used_muTracks_index){
      ...
      unsigned int imu1(imu.first); 
      unsigned int imu2(imu.second);
      ...
      for(const reco::TransientTrack & trk : KTrack)
      {
       //  std::cout<<"K "<<trk.track().pt()<<" eta "<<trk.track().eta()<<" phi "<<trk.track().phi()<<std::endl;
        ...
        for ( auto & x : {vk.Pt(),vk.Eta(),vk.Phi()}) 
          tempK.push_back(x);
        ...
        if (!RefitTracks)
        {
	      for (auto & x : {(vmu1+vmu2+vk).Pt(),(vmu1+vmu2+vk).Eta(),(vmu1+vmu2+vk).Phi()} ) 
	        tempBpt.push_back(x);      
        } 
        else
        {
          for (auto & x : {fitter.Mother_Momentum(RefitTracks).perp(),fitter.Mother_Momentum(RefitTracks).eta()} ) 
            tempBpt.push_back(x);
        ...
        for (auto & x : {fitter.Mother_XYZ().x(),fitter.Mother_XYZ().y(),fitter.Mother_XYZ().z()} ) 
          tempBx.push_back(x);
        
        nt.NRb_x_y_z.push_back(tempBx);
     
        for (auto & x : {fitter.Mother_XYZError().cxx(),fitter.Mother_XYZError().cyy(),fitter.Mother_XYZError().czz()} ) 
          tempBex.push_back(x);
        
        nt.NRb_ex_ey_ez.push_back(tempBex); 
      
        for (auto & x : {fitter.Mother_PtError(),fitter.Mother_EtaError(),fitter.Mother_PhiError()}) 
          tempBept.push_back(x);
        ...
        nt.NRb_lep1Id.push_back(imu1);
        nt.NRb_lep2Id.push_back(imu2);
        nt.NRb_mll.push_back((vmu1+vmu2).M()); 
        nt.NRb_vtx_index.push_back(-1);
